    ...
    for j = 1,#pl do
      if not users[pl[j].name] then
        ...
        if not white_list[pl[j].name] then 
          ...
          gpu.set(2, count+1, count..'. '..pl[j].name .. " x:" .. math.floor(pl[j].x + 1) .. 
                 " y:" .. math.floor(pl[j].y) .. " z:" .. math.floor(pl[j].z))
          
          if com.isAvailable("particle") then
            for i = 1, 5 do
              com.particle.spawn("lava", pl[j].x - -72.5, pl[j].y - 64, pl[j].z - 250.5)
            end
          end
        end 
      end
